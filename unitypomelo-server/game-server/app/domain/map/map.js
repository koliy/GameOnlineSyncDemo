...
var buildFinder = require('../../util/pathfinding').buildFinder;
var formula = require('../../consts/formula');

var Map = function(opts){
	
	this.mapPath = process.cwd() + opts.path;//"/config/map/desert.json"

	this.map = null; //地图保存的'objectgroup'对象数据
	this.weightMap = null;//保存的地图格子上碰撞体标签
	this.name = opts.name;
...
pro.init = function(opts){
	...
	if(!map) logger.error('Load map failed! ');
	else{
		this.configMap(map);
		this.id = opts.id;
...
pro.configMap = function(map){
	this.map = {};
	var layers = map.layers;
	for(var i =0;i<layers.length;i++){
		var layer = layers[i];
		if(layer.type === 'objectgroup') this.map[layer.name] = configObjectGroup(layer.objects);
	}
};
...
pro.getCollision = function(){
	return this.map.collision; //json数据里面的collision数据
};

pro.getMobZones = function(){
	if(!this.map){
		logger.error('map not load');
		return null;
	}
	
	return this.map.mob;//json数据里面的mob数据
};

/***
 * 解析地图碰撞体格子数据
 */
pro.initWeightMap = function(){
	var collisions = this.getCollision(); //获取文件里面设置的碰撞区域数据
		...
		if(!!points && points.length > 0){
			...
			for(x = x1;x<x2;x++){
				for(y=y1;y<y2;y++){
					...
					p3 = {x:p.x -l,y:p.y +l};
					p4 = {x:p.x +l,y:p.y +l};
					//检测当前格子的4个中点是否在多边形内，有1个点满足，则此格子为碰撞格子。
...
pro.getWeightMap = function(collisions){
	...
	for(x =0;x<this.rectW;x++){
		var row = [];
		for(y =0;y<this.rectH;y++) row.push(1);
		map.push(row);
	}
	...
	for(x =0;x<collisions.length;x++){
		...
		for(var j =0;j<array.length;j++){
			var c = array[j];
			for(var k=0;k<c.length;k++) map[x][c.start+k] = Infinity;
		}
	}
	return map;
...
pro.initCollisions = function(){
	...
	for(var x =0;x<this.weightMap.length;x++){
		...
		for(var y =0;y<length;y++){
			...
			}else if(flag && (y === length - 1)){ //当此行最后一格也是碰撞体
				flag = false;
...
pro.isReachable = function(x,y){
	...
	try{
		...
		if(!this.weightMap[x1] || !this.weightMap[x1][y1]) return false;
	}catch (e) {
...
pro.getBornPlace = function(){
	var bornPlace = this.map.birth[0];//获取配置表中的出生点
	if(!bornPlace) bornPlace = this.map.transPoint; //没有则获取传送点
	if(!bornPlace) return null;
	return bornPlace;
};

pro.getBornPoint = function(){
	var bornPlace = this.getBornPlace();
	var pos = {
		x:bornPlace.x + Math.floor(Math.random() * bornPlace.width),
		y:bornPlace.y + Math.floor(Math.random() * bornPlace.width)
	};
	return pos;
};

pro.findPath =function(x,y,x1,y1,useCache){
	...
	if(!this.isReachable(x1,y1)){
		logger.warn('The end point is not reachable! start:(%j,%j)',x1,y1);
		return null;
	}
...
pro.verifyPath = function(path){
	...
	for(i=0;i<path.length;i++){
		if(!this.isReachable(path[i].x,path[i].y)) return false;
	}
	
	for(i=1;i<path.length;i++){
		if(!this._checkLinePath(path[i-1].x,path[i-1].y,path[i].x,path[i].y)){
			logger.error('illigle path  ! i: %j,paht[i-1] :%j,path[i]:%j',i,path[i-1],path[i]);
			return false;
		}
	}
...
pro.genPos = function(pos,range){
	var result = {};
	var limit = 10;
	for(var i =0;i<limit;i++){
		var x = pos.x + Math.random()*range - range*0.5;
		var y = pos.y + Math.random()*range - range*0.5;
		if(this.isReachable(x,y)){
			result.x = x;
			result.y = y;
			return result;
