...
//两点圆心O（0,0）和圆上的某点P（x,y）之间的距离就是半径r．
...
var createPriorityQueue = function(cmpPriority){
	...
	obj.enqueue = function(e){
		this.arr.push(e);
		//把最后入队的数据和[0]比较，其中最大值保存在[0].
		var idx = this.arr.length - 1;
		var parentIdx = floor((idx - 1) / 2);
		while( 0 <= parentIdx){
			if(cmpPriority(this.arr[idx],this.arr[parentIdx]) <= 0) break;
	...
	obj.dequeue = function(){
		...
		while(true){
			...
			var targetPos = idx;
			if(leftChildIdx  < this.arr.length && cmpPriority(this.arr[targetPos],this.arr[leftChildIdx]) < 0)
				targetPos = leftChildIdx;
			
			if(rightChildIdx < this.arr.length && cmpPriority(this.arr[targetPos],this.arr[rightChildIdx]) < 0)
	...
	var finder = function(sx,sy,gx,gy){
		...
		while(0< queue.length()){
			...
			var processReachable = function(theX,theY,weight){
				if(weight >= CAN_NOT_MOVE) return; // 不可能走
...
// var q = createPriorityQueue(function(a,b){return a-b;});
