...
namespace PureMVC.Core
{
    ...
    public class View : IView
    {
        ...
        {
            //   Log.log("View RegisterMediator 00000000 ");
            lock (m_mediatorMap)
            {
				...
				if (interests != null)
				{
					//Log.log("View RegisterMediator  List count: " + interests.Count);
					for (int i = 0, j = interests.Count; i < j; i++)
					{
						if (interests[i] != null)
						{
							Observer((string)interests[i], observer);
						}
						else
						{
							//Log.log("View RegisterMediator  List i null null  " + i);
						}
					}
				}
				else
				{
					//Log.log("View RegisterMediator  List null null null ");
				}
            }
            // alert the mediator that it has been registered
            mediator.OnRegister();
        }
        void Observer(string nameId, IObserver observer)
        {
			if (!String.IsNullOrEmpty(nameId))
			{
				//Log.log("RegisterObserver name:" + nameId);
				RegisterObserver(nameId, observer);
			}
			else
			{
				//Log.log("RegisterObserver name null null null");
			}
        }
        ...
        public virtual IMediator RemoveMediator(string mediatorName)
        {
            lock (m_mediatorMap)
            {
				...
				if (interests != null)
				{
					//Log.log("View RemoveMediator  List count: " + interests.Count);
					for (int i = 0, j = interests.Count; i < j; i++)
					{
						if (interests[i] != null)
						{
							Remove((string)interests[i], mediator);
						}
						else
						{
							//Log.log("View RemoveMediator  List i null null  " + i);
						}
					}
				}
				else
				{
					//Log.log("View RemoveMediator  List null null null ");
				}
        ...
        void Remove(string nameId, object mediator)
        {
			if (!String.IsNullOrEmpty(nameId))
			{
				//Log.log("RemoveObserver name:" + nameId);
				RemoveObserver(nameId, mediator);
			}
			else
			{
				//Log.log("RemoveObserver name null null null");
			}
